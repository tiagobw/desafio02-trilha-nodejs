...
function checksCreateTodosUserAvailability(request, response, next) {
  const isFreePlanExceeded =
    !request.user.pro && request.user.todos.length >= 10;

  if (request.user && (!isFreePlanExceeded || request.user.pro)) {
    next();
  }

  if (isFreePlanExceeded) {
    return response
      .status(403)
      .json({ error: "The Free plan accepts a maximum of 10 Todos." });
  }
}
